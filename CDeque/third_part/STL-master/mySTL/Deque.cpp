...
namespace STL {
	namespace Detail {
		...
		deque_iterator<T> operator +(const deque_iterator<T>& it
			...
			if (n <= leftSize) {//前进n步后依然在当前桶内   （判断是否在当前桶）
				retIt.cur_ += n;
			}
			else {
				n -= leftSize;
	...
	T * deque<T, Alloc>::getANewBuck() {
		return dataAllocator::allocate(getBuckSize());//构造空间
	}
	template<class T, class Alloc>
...
//		uninitialized_copy(first, last, start_);
	...
	void deque<T, Alloc>::creat_map_and_nodes(size_t num_elements) {
		...
		map_size_ = max(size_type(2), num_bucks + 2);					//分配区块个数
		map_ = mapAllocator::allocate(map_size_);						//分配空间
		size_type newStartIndex = (map_size_ - num_bucks) / 2;			//确定起始索引
		size_type newFinishIndex = newStartIndex + num_bucks - 1;		//确定末尾索引
		
		for (auto i = newStartIndex; i <= newFinishIndex; ++i) {
			map_[i] = getANewBuck();			//循环申请缓冲区块
		}
		start_.mapIndex_ = newStartIndex;
		finish_.mapIndex_ = newFinishIndex;
		start_.cur_ = map_[newStartIndex];									 //头指针位置
		finish_.cur_ = map_[newFinishIndex] + num_elements % getBuckSize();  //尾指针位置
		start_.container_ = finish_.container_ = this;
	}
	template<class T, class Alloc>
	void deque<T, Alloc>::reallocateMap(size_t nodes_to_add, bool add_at_front) {
		size_t old_num_mapNodes = finish_.mapIndex_ - start_.mapIndex_ + 1;  //旧的节点数
	...
	deque<T, Alloc>::deque()		//1、
		:map_size_(0), map_(nullptr) {			//c++11 引入nullptr
	}
	...
	//中间插入  在首端或者末端 添加元素 ，拷贝起始位置至 插入位置的元素， 将要插入的数据写入指定位置
	template<class T, class Alloc>
	int deque<T, Alloc>::insert(unsigned int nIndex, void *pData) {
		...
		else
		{
			...
			{
				push_front(front());				//给前面加 1
				iterator __front1 = start_;
				--__front1;							//目标容器
				iterator __front2 = __front1;
				++__front2;							//起始位置
				//nIndex = this->start_ + __index;	
				iterator __pos1 = start_ + nIndex;	//目标位置
				++__pos1;
				STL::copy(__front2, __pos1, __front1);    //参数分别为其实位置 目标位置  拷贝容器
	...
	//删除中间数据 判断在末端还是首段删除数据，拷贝其实位置至 索引 ，删除尾部或头部数据
	template<class T, class Alloc>
	...
	template<class T, class Alloc>
	void deque<T, Alloc>::init() {			//如果是空  初始化内存
		map_size_ = 2;                      //初始化俩个map指针
