...
namespace STL {
	...
	namespace Detail {
		//1、双端队列指针类 deque_iterator
		template<class T>
		class deque_iterator :public iterator<random_access_iterator_tag, T> {//继承自包含标准迭代器typedef的iterator
			template<class T, class Alloc>
			friend class deque;							//定义友元  
			typedef deque<T>* containerPtr;
		private:
			containerPtr container_;						//定义指向队列的指针
			size_t    mapIndex_;							//
			T*           cur_;								//此迭代器所指缓存区的当前位置
		public:
			//构造相关
			deque_iterator()											//1、无参构造
				:mapIndex_(-1), cur_(nullptr), container_(nullptr) {}    //实列化队列默认走无参构造

			deque_iterator(size_t index, T *ptr, containerPtr container)//2、有参构造
				:mapIndex_(index), cur_(ptr), container_(container) {}
			deque_iterator(const deque_iterator& it)					//3、有参构造
				:mapIndex_(it.mapIndex_), cur_(it.cur_), container_(it.container_) {}
			
			deque_iterator& operator = (const deque_iterator& it);		//赋值运算符重载

			//符号重载
			deque_iterator& operator ++ ();								//重载前置++
			deque_iterator operator ++ (int);							//重载后置++
	...
	private:
		...
		map_Pointer map_;				//3、指向map（map为一段连续空间）,其内部元素为指针，每个指针指向一个缓存区
		size_type map_size_;			//4、map内部指针的个数
